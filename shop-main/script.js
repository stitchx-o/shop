...
function handleConfirmCustomOrder() {
    ...
    // إعداد FormData
    const formData = new FormData();
    formData.append('product', document.querySelector('.custom-product-title') ? document.querySelector('.custom-product-title').textContent : '');
    formData.append('priceBeforeFees', document.querySelector('.new-price') ? document.querySelector('.new-price').textContent : '');
    formData.append('name', name);
    formData.append('phone', phone);
    formData.append('wilaya', state);
    ...
    // إضافة صورة التصميم إذا رفعها المستخدم
    const designFileInput = document.getElementById('designFileInput');
    if (designFileInput && designFileInput.files && designFileInput.files.length > 0) {
        formData.append('design', designFileInput.files[0]);
    }

    fetch('https://script.google.com/macros/s/AKfycbxGO9eO8rAQb23kv7PaWimKMx8asdlMGI7cPQwmbSxZAW8cWBnhCuqkP2-wbbyc0Eerog/exec', {
        method: 'POST',
        body: formData
    })
    .then(res => res.json())
